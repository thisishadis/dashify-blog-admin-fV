...
export default function AllPosts() {
  ...
  const {
    ...
    queryKey: ["products", searchParams.get("page") || 1, rowsPerPage],
    queryFn: () =>
      fetchProducts(rowsPerPage, (page || 0) * rowsPerPage), // Adjusting for pagination
  });
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    setSearchParams({ page: newPage + 1 }); // Adjusting for pagination
  };

  // const handleChangeRowsPerPage = (event) => {
  //   setRowsPerPage(parseInt(event.target.value, 10));
  //   setPage(0);
  //   setSearchParams({ page: 1, rowsPerPage: event.target.value }); // Adjusting for pagination
  // };
  const handleChangeRowsPerPage = (event) => {
    const newRowsPerPage = parseInt(event.target.value, 10);
    setRowsPerPage(newRowsPerPage);
    setPage(0);
    setSearchParams({ page: 1, rowsPerPage: newRowsPerPage }); // Adjusting for pagination
  };
              ...
              {isLoading ? (
                ...
                products.map((product) => (
                  <tr key={product.id} className="cursor-pointer hover">
                    ...
                    >
                      <img src={VISI} alt="Visibility Icon" />
                    </td>
                  </tr>
                ))
              ) : (
